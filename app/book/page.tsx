...
export default function BookingPage() {
  ...
  return (
    <div className="p-4">
      ...
      <p><strong>Location:</strong> {printer.location || 'Unknown'}</p>
      <p><strong>Description:</strong> {printer.description || 'None provided'}</p>
      <p><strong>Materials Supported:</strong> {printer.materials?.join(', ') || 'N/A'}</p>
      <p><strong>Max Print Size:</strong> {printer.build_volume || 'N/A'}</p>
      <p><strong>Hourly Rate:</strong> {printer.price_per_hour ? `$${parseFloat(printer.price_per_hour)}` : '$N/A'}</p>
      <p><strong>Availability:</strong> {printer.availability || 'Not specified'}</p>
